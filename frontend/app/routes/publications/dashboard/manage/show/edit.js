...
import ResetScroll from "frontend/mixins/resetscroll";
...
export default Ember.Route.extend(AuthenticatedRouteMixin, ResetScroll, {
  ...
  model: function () {
    ...
    return Ember.RSVP.hash({
      ...
      publicationIdentifierCodes: this.store.find(
        "publication_identifier_code"
      ),
  ...
  setupController: function (controller, models) {
    ...
    models.publication.publication_links =
      models.publication.publication_links.map((item) => {
    ...
    controller.set(
      "publicationIdentifierCodes",
      models.publicationIdentifierCodes
    );
    controller.set("publicationTypes", models.publicationTypes);
    ...
    if (models.publication) {
      if (models.publication.authors) {
        ...
        controller.set(
          "arrOfAuthorsFromImport",
          models.publication.authors_from_import
        );
      }
    }
    let tempAuthorArr = [];
    ...
    var publicationType = models.publicationTypes.findBy(
      "id",
      models.publication.publication_type_id
    );
    if (publicationType) {
    ...
    if (models.publication.publication_type_suggestion) {
      ...
      controller.set(
        "suggestedPublicationType",
        models.publication.publication_type_suggestion
      );
    } else {
  ...
  returnToArguments: Ember.computed(
    "returnTo",
    "returnToModels",
    "returnToQueryParams",
    function () {
  ...
  actions: {
    ...
    cancelEdit: function () {
      if (this.get("hasReturnTo")) {
        ...
        this.get("controller").get("publication.process_state") === "PREDRAFT"
      ) {
        this.transitionTo("publications.dashboard.manage.published");
      } else {
        this.transitionTo(
          "publications.dashboard.manage.show",
          this.controller.get("publication.id")
        );
      }
    },
    ...
    savePublication: function (isDraft) {
      if (!isDraft) {
        ...
        if (!authorAffiliated) {
          let continueSave = confirm(
            this.get("i18n").t(
              "publications.dashboard.manage.show.edit.confirm"
            )
          );
          if (!continueSave) {
            return;
      ...
      this.get("controller.publication.publication_links")
        .sortBy("position")
        .forEach((link, index) => {
          link.set("position", index);
        });
      ...
      let savePublication = new Ember.RSVP.Promise((resolve, reject) => {
        let successHandler = (model) => {
          let message = isDraft
            ? this.get("i18n").t(
                "publications.dashboard.manage.show.edit.saveDraftSuccess"
              )
            : this.get("i18n").t(
                "publications.dashboard.manage.show.edit.publishSuccess"
              );
        ...
        let errorHandler = (errors) => {
          ...
          if (!isDraft && this.controller.get("publication.draft_id")) {
            this.controller.set(
              "publication.id",
              this.controller.get("publication.draft_id")
            );
        ...
        this.get("controller")
          ...
          .then(() => {
            ...
            this.store
              .save(resource, this.controller.get("publication"))
              .then((model) => {
                if (!model) {
                  errorHandler(
                    this.get("i18n").t(
                      "publications.dashboard.manage.show.edit.systemError"
                    )
                  );
                } else if (model.error) {
