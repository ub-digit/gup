...
export default Ember.Controller.extend({
  ...
  updateCategoryObjects: Ember.observer('publication.category_current_type.[]', function(){
    ...
    if (this.get('publication.category_current_type')) {
      this.get('publication.category_current_type').forEach((item) => {
    ...
    this.get('categoryObjectsList').forEach((item) => {
      if (this.get('publication.category_current_type').indexOf(item.id) === -1) {
