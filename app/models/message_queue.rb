...
class MessageQueue
  def self.send_update_to_queue publication
    ...
    begin
      ...
      Rails.logger.warn "Error posting update document with publication id=#{publication.id} to MQ at host #{APP_CONFIG['mq_settings']['base_url']}"
    end
  end
  ...
  def self.send_delete_to_queue publication_id
    ...
    begin
      ...
      Rails.logger.warn "Error posting delete document with publication id=#{publication_id} to MQ at host #{APP_CONFIG['mq_settings']['base_url']}"
    end
  end
end
