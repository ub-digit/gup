namespace :gup_migrations do
  desc "Add new category_type for SSIF_25"
  task :add_category_type_for_ssif_2025 => :environment do
  ...
  desc "Change category field name in database for SSIF 2025"
  task :change_category_field_name_for_ssif_25 => :environment do
  ...
  task :map_categories_to_ssif_2025 => :environment do
    ...
    category_mappings.each do |k, v|
      hsv_11_category = Category.where(category_type: 'HSV_11').find_by(svepid: k)
      ssif_25_category = Category.where(category_type: 'SSIF_25').find_by(svepid: v)
